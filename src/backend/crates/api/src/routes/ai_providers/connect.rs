...
use crypto::{envelope::encrypt_key, kms::LocalKms};
...
pub async fn handler(
    ...
    let provider: Box<dyn AiProvider> = match body.provider.as_str() {
        ...
        _ => return Err(ApiError(lh_core::AppError::BadRequest(
    ...
    provider.validate_key(&body.api_key).await.map_err(|e| match e {
        AiRouterError::AiProviderKeyInvalid =>
            ApiError(lh_core::AppError::Conflict("API key is invalid or revoked".into())),
        other =>
            ApiError(lh_core::AppError::Internal(other.to_string())),
    })?;

    // 2. Envelope-encrypt the key
    let kms = LocalKms::from_env()
        .map_err(|e| ApiError(lh_core::AppError::Internal(e.to_string())))?;
