...
impl AiRouter {
    ...
    pub async fn dispatch(
        ...
        // 2. Budget check
        let spent = crate::budget::monthly_spend(&self.pool, user_id).await?;
        if spent >= cred.monthly_cap_usd {
            return Err(AiRouterError::AiProviderBudgetExceeded);
        }

        // 3. Decrypt API key
        let kms = crypto::LocalKms::from_env()
            .map_err(|e| AiRouterError::Crypto(crypto::EnvelopeError::Kms(e)))?;
        let plaintext: Zeroizing<Vec<u8>> = crypto::decrypt_key(
            &kms,
            &cred.ciphertext,
            &cred.nonce,
            &cred.encrypted_dek,
