...
impl AiProvider for OpenAiProvider {
    ...
    async fn validate_key(&self, api_key: &str) -> Result<(), AiRouterError> {
        let req = ChatRequest {
            messages:        vec![crate::ChatMessage { role: "user".into(), content: "hi".into() }],
            system:          None,
            max_tokens:      1,
            temperature:     0.0,
...
mod tests {
    ...
    async fn http_401_returns_key_invalid() {
        ...
        assert!(matches!(err, crate::AiRouterError::AiProviderKeyInvalid));
    }
    ...
    async fn http_429_returns_upstream() {
        ...
        assert!(matches!(err, crate::AiRouterError::AiProviderUpstream(_)));
    }
}
